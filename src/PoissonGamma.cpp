...
arma::vec gradient_descent_multiple_beta_psi_mu2(arma::vec Y, arma::mat X, arma::vec beta, double psi, double mu2, double gamma,
  ...
  for(int i = 0; i < steps; ++i){
    ...
    double stepsize = 0;
    if(abs(gradient(2)) >= 0.0001) {
      stepsize = select_stepsize_for_multiple_beta(Y, X, beta, Xbeta, 0, m_gra_beta, ll, psi, mu2, n, gamma, sum_log_factorial_Y, down);
    }
    
    beta_prime = beta + stepsize*m_gra_beta;
    mu2_prime = mu2 + stepsize*gradient(1);
    //  for(int j = 0; j < k; ++j){
    ...
    m_gra_beta = gradient(arma::span(2, 1 + k));
